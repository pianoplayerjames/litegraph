/**
 * LiteGraph - Core module containing constants, configuration, and static utility methods
 * @module LiteGraph
 */
...
const LiteGraph = {
    ...
    // Placeholder properties for classes that will be set later
    LGraph: null,
    LGraphNode: null,
    LGraphCanvas: null,
    LLink: null,
     ...
     * @param {Function} base_class - class containing the structure of a node
     */
    registerNodeType: function(type, base_class) {
        ...
        // extend class - use Object.getOwnPropertyNames to include non-enumerable methods (ES6 class methods)
        if (LiteGraph.LGraphNode) {
            const protoProps = Object.getOwnPropertyNames(LiteGraph.LGraphNode.prototype);
            for (let i = 0; i < protoProps.length; i++) {
                const prop = protoProps[i];
                if (prop !== 'constructor' && !base_class.prototype[prop]) {
                    base_class.prototype[prop] = LiteGraph.LGraphNode.prototype[prop];
                }
            }
        }
     ...
     * @param {string|Function} type - name of the node or the node constructor itself
     */
     ...
     * @method registerNodeAndSlotType
     * @param {string|Function} type - name of the node or the node constructor itself
     * @param {string} slot_type - name of the slot type (variable type), eg. string, number, array, boolean, ..
     * @param {boolean} out - true if output slot, false if input slot
     */
     ...
     * @return {LGraphNode|null} the created node or null if failed
     */
     ...
     * @method getNodeType
     * @param {string} type - full name of the node class. p.e. "math/sin"
     * @return {Function|null} the node class
     */
     ...
     * @method reloadNodes
     * @param {string} folder_wildcard - wildcard pattern for scripts to reload
     */
    ...
    /**
     * Clone an object (deep copy using JSON)
     * @method cloneObject
     * @param {Object} obj - object to clone
     * @param {Object} target - [optional] target object to copy properties to
     ...
     * @param {string} type_a - first type
     * @param {string} type_b - second type
     ...
     * @return {FileReader|Promise|null} returns the object used to fetch
     */
     ...
     * @param {Window} ref_window - reference to window (defaults to window)
     */
    ...
    /**
     * Extend a class with properties from another class
     * @method extendClass
     * @param {Function} target - target class to extend
     * @param {Function} origin - source class to copy from
     */
     ...
     * @param {Function} func - function to extract parameter names from
     ...
     * @param {HTMLElement} oDOM - DOM element to attach listener to
     * @param {string} sEvIn - event type (down, up, move, over, out, enter, leave, cancel, gotpointercapture, lostpointercapture)
     * @param {Function} fCall - callback function
     ...
     * @param {HTMLElement} oDOM - DOM element to remove listener from
