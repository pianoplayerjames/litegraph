/**
 * LGraphCanvas - ES6 Class Module
 *
 * This class handles all canvas rendering, mouse/keyboard input, and visual editing of the node graph.
 * Converted from constructor function to ES6 class.
 */

import { DragAndScale } from './DragAndScale.js';

// Module-level variable for LiteGraph reference - set via setLiteGraph()
let LiteGraph = null;

// Reusable temp buffers for various calculations (avoiding allocations)
...
class LGraphCanvas {
    ...
    static link_type_colors = { "-1": null, number: "#AAA", node: "#DCA" };
    static gradients = {};
    static node_colors = {
    ...
    _doNothingdoNothing(e) {
    	//console.log("pointerevents: _doNothing "+e.type);
        e.preventDefault();
        return false;
    };
    _doReturnTruedoNothing(e) {
        e.preventDefault();
        return true;
    };
	...
	*/
    ...
    static showMenuNodeOptionalInputs(
        ...
        let input_options = node.optional_inputs;
        if (node.onGetInputs) {
            input_options = node.onGetInputs();
        }

        let entries = [];
        if (input_options) {
            for (let i=0; i < input_options.length; i++) {
                let entry = input_options[i];
                if (!entry) {
                    entries.push(null);
                    continue;
                }
    ...
    static showMenuNodeOptionalOutputs(
        ...
        let output_options = node.optional_outputs;
        if (node.onGetOutputs) {
            output_options = node.onGetOutputs();
        }

        let entries = [];
        if (output_options) {
            for (let i=0; i < output_options.length; i++) {
                let entry = output_options[i];
                if (!entry) {
                    //separator?
                    entries.push(null);
                    continue;
            ...
            if (that.onSearchBox) {
				...
				function inner_test_filter( type, optsIn = {} )
				{
                    ...
                    if(options.do_type_filter && !opts.skipFilter){
                        ...
                        let sVOut = sOut.value;
                        if (opts.outTypeOverride!==false) sVOut = opts.outTypeOverride;
                        //if (sV.toLowerCase() == "_event_") sV = LiteGraph.EVENT; // -1
                        
                        if(sOut && sVOut){
                            //console.log("search will check filter against "+sVOut);
                            if (LiteGraph.registered_slot_out_types[sVOut] && LiteGraph.registered_slot_out_types[sVOut].nodes){ // type is stored
                                //console.debug("check "+sType+" in "+LiteGraph.registered_slot_out_types[sVOut].nodes);
                                let doesInc = LiteGraph.registered_slot_out_types[sVOut].nodes.includes(sType);
                                if (doesInc!==false){
                                    //console.log(sType+" HAS "+sVOut);
                                }else{
                                    /*console.debug(LiteGraph.registered_slot_out_types[sVOut]);
                                    console.log(+" DONT includes "+type);*/
                                    return false;
                                }
                            }
    ...
    closePanels(){
        let nodePanel = document.querySelector("#node-panel");
		if(nodePanel)
			nodePanel.close();
...
function compareObjects(a, b) {
    ...
    // Helper functions - will be assigned to LiteGraph in setLiteGraph()
...
/**
 * Set the LiteGraph reference (must be called before using LGraphCanvas)
 * @param {Object} lg - LiteGraph instance
 */
LGraphCanvas.setLiteGraph = function(lg) {
    LiteGraph = lg;
    LGraphCanvas.LiteGraph = lg;

    // Assign helper functions to LiteGraph namespace
    if (lg) {
        lg.compareObjects = compareObjects;
        lg.distance = distance;
        lg.colorToString = colorToString;
        lg.isInsideRectangle = isInsideRectangle;
        lg.growBounding = growBounding;
        lg.isInsideBounding = isInsideBounding;
        lg.overlapBounding = overlapBounding;
        lg.hex2num = hex2num;
        lg.num2hex = num2hex;
    }
};
